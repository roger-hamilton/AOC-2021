...
const parseBalls = (line: string) => line.split(',').map(toInt);
...
const allRuns = (balls: number[]) => [...Array(balls.length).keys()]
  .map(i => balls.slice(0, i + 1))
...
const part2 = (boards: number[][][], balls: number[]) => {
  const loosers = allRuns(balls)
    .reduce((l, run) => l.length < 2 ? l : l.filter(b => !isWinner(b, run)), boards);
